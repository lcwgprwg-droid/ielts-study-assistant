...
function importRecordText(text) { try { const report = importRecord(JSON.parse(text)); document.querySelector("#recordJson").value = ""; document.querySelector("#recordMessage").textContent = `已导入${scopeLabel(report.scope)}记录，并更新仪表盘。`; toast("学习记录已导入"); } catch (error) { document.querySelector("#recordMessage").textContent = error.message; } }
document.querySelector("#recordForm").addEventListener("submit", (event) => { event.preventDefault(); importRecordText(document.querySelector("#recordJson").value); });
document.querySelector("#clipboardImport").addEventListener("click", async () => { try { const text = await navigator.clipboard.readText(); document.querySelector("#recordJson").value = text; importRecordText(text); } catch { document.querySelector("#recordMessage").textContent = "浏览器未授予剪贴板权限。请将记录卡粘贴到上方输入框后导入。"; } });
